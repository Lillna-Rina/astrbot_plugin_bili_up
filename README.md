...
```
/b站绑定 ─▶ 生成B站登录二维码 ─▶ App扫码并确认 ─▶ Cookie 持久化(按QQ号, 存于AstrBot库)
/上传视频 ─▶ 发送视频文件 ─▶ 标题 → 原创/转载 → (转载来源) → 分区 → 简介 → 标签
...
| 命令 | 说明 |
|---|---|
| `/b站绑定` | 生成二维码扫码登录（3 分钟有效），成功后自动保存 Cookie |
| `/b站状态` | 校验当前绑定账号是否有效（昵称 / mid） |
| `/b站解绑` | 删除本地 Cookie |
| `/b站分区` | 查看常用分区(tid)对照表 |
| `/上传视频` | 投稿主流程；任意步骤回复【取消】终止，回复【投稿】确认提交 |
...
3. AstrBot 会自动安装 `requirements.txt` 依赖；若失败，请在 AstrBot 的 Python 环境手动执行：
   ...
   ```
4. 重启 AstrBot，向机器人私聊发送 `/b站绑定` 开始使用。
...
- Cookie 通过 `Star` 自带的 `put_kv_data/get_kv_data` 持久化到 AstrBot 数据库（不落明文文件）；
...
- **Cookie 安全**：SESSDATA 属于完整登录态，插件仅保存在 AstrBot 数据库并按 QQ 号隔离，请勿将数据库/日志外泄；
